...
def detectar_comuna_por_gps(lat, lon, indice):
    """Detecta la comuna donde cae el punto GPS usando los polígonos PRC disponibles."""
    ...
    for clave, info in indice.items():
        try:
            if not info.get("archivo"):
                continue

            gdf = cargar_shp(info["archivo"])
...
st.info(
    "Puedes usar el GPS del celular o la ubicación aproximada del computador. "
    "El navegador solicitará permiso antes de compartir la ubicación. "
    "Al detectar la ubicación, HOMOLOGADOR intentará cambiar automáticamente la comuna seleccionada."
)

if get_geolocation is None:
    st.warning("GPS no disponible. Agrega `streamlit-js-eval` en requirements.txt para activar esta función.")
else:
    if st.button("📍 Detectar mi ubicación actual"):
        ubicacion = get_geolocation()

        if ubicacion and "coords" in ubicacion:
            lat_gps = ubicacion["coords"]["latitude"]
            lon_gps = ubicacion["coords"]["longitude"]
            precision = ubicacion["coords"].get("accuracy", None)

            st.session_state.lat = lat_gps
            st.session_state.lon = lon_gps
            st.session_state.gps_precision_m = precision

            comuna_gps = detectar_comuna_por_gps(
                lat_gps,
                lon_gps,
                indice
            )
            ...
            if comuna_gps:
                ...
                st.session_state.gps_msg = f"Comuna detectada automáticamente: {indice[comuna_gps]['nombre']}"
            else:
                st.session_state.gps_msg = (
                    "Ubicación detectada, pero no se pudo identificar automáticamente la comuna. "
            ...
            st.warning(
                "No se pudo obtener la ubicación. Revisa los permisos de ubicación del navegador "
                "o intenta nuevamente desde el celular."
            )

if st.session_state.gps_msg:
    st.success(st.session_state.gps_msg)
...
if st.session_state.lat is not None and st.session_state.lon is not None:
    folium.Marker(
        ...
        popup=(
            "Punto seleccionado"
            if not st.session_state.gps_precision_m
            else f"Punto seleccionado<br>Precisión GPS aprox.: {round(float(st.session_state.gps_precision_m), 1)} m"
        ),
