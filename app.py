...
def detectar_comuna_por_punto(lat, lon, indice, gdf_prms_uso_total=None, gdf_prms_lu_total=None):
    """
    Detecta automáticamente la comuna de un punto GPS.
    Primero intenta intersectar con los PRC disponibles.
    Si no encuentra PRC, usa PRMS_USO_Suelo o PRMS_LU como respaldo.
    """
    ...
    # 1) Buscar dentro de los PRC disponibles
    for clave, info in indice.items():
        ...
        try:
            ...
            if not match.empty and not pd.isna(match.iloc[0].get("index_right")):
                return clave

        except Exception:
            continue

    # 2) Respaldo con capas PRMS totales cuando la comuna no tiene PRC cargado
    for gdf_total in [gdf_prms_uso_total, gdf_prms_lu_total]:
        try:
            if gdf_total is None or gdf_total.empty:
                continue

            gdf_total = normalizar_columnas(gdf_total)
            ...
            if match.empty or pd.isna(match.iloc[0].get("index_right")):
                continue

            comuna_detectada = str(match.iloc[0].get("COMUNA", "")).strip()
            clave_detectada = normalizar(comuna_detectada)

            if clave_detectada in indice:
                return clave_detectada
...
if "auto_gps_on_load" not in st.session_state:
    st.session_state.auto_gps_on_load = False
...
# El selector queda sincronizado con la comuna detectada por GPS.
if "comuna_selector" not in st.session_state:
    st.session_state.comuna_selector = st.session_state.comuna_actual

if (
    st.session_state.comuna_actual in comunas_ordenadas
    and st.session_state.comuna_selector != st.session_state.comuna_actual
):
    st.session_state.comuna_selector = st.session_state.comuna_actual

default_index = 0

if st.session_state.comuna_selector in comunas_ordenadas:
    default_index = comunas_ordenadas.index(st.session_state.comuna_selector)
...
# Si el cambio fue manual, se limpian las coordenadas anteriores para evitar homologar con otra comuna.
...
st.info(
    ...
    "El navegador pedirá autorización antes de compartir la ubicación. "
    "Al detectar el punto, HOMOLOGADOR intentará cambiar automáticamente a la comuna correcta."
)

col_gps1, col_gps2 = st.columns([2, 1])

with col_gps1:
    st.session_state.auto_gps_on_load = st.toggle(
...
if get_geolocation is None:
    st.warning(
        "Para activar GPS instala la dependencia `streamlit-js-eval` en requirements.txt."
    )
else:
    if st.session_state.auto_gps_on_load or boton_gps:
        ...
        if ubicacion and "coords" in ubicacion:
            ...
            if lat_gps is not None and lon_gps is not None:
                st.session_state.lat = float(lat_gps)
                st.session_state.lon = float(lon_gps)
                ...
                if comuna_gps:
                    ...
                    if precision:
                        st.success(
                            f"Ubicación GPS detectada. Precisión aproximada: {round(float(precision), 1)} m. "
                            f"Comuna detectada automáticamente: {indice[comuna_gps]['nombre']}."
                        )
                    else:
                        st.success(
                    ...
                    st.warning(
                        "No se pudo detectar automáticamente la comuna del punto GPS. "
                        "Selecciona la comuna manualmente para una homologación correcta."
                    )
...
if st.session_state.lat is not None and st.session_state.lon is not None:
    folium.Marker(
        ...
        popup=(
            "Punto seleccionado"
            + (
                f"<br>Precisión GPS aprox.: {round(float(st.session_state.gps_precision_m), 1)} m"
