...
if st.button("Buscar dirección y homologar"):
    if not direccion.strip():
        ...
        # Se fuerza la búsqueda a la Región Metropolitana para evitar coincidencias en otras ciudades/comunas.
        consulta = f"{direccion}, Región Metropolitana, Chile"

        try:
            with st.spinner("Buscando dirección en la Región Metropolitana..."):
            ...
            if location:
                ...
                if comuna_dir and comuna_dir in indice:
                    st.session_state.comuna_actual = comuna_dir
                    st.success(
                        f"Dirección encontrada. Comuna detectada automáticamente: {indice[comuna_dir]['nombre']}."
                    )
                    st.rerun()
                else:
                    st.success(
                        f"Dirección encontrada. Coordenadas: {lat_dir}, {lon_dir}. "
                        "Selecciona la comuna manualmente si el resultado no corresponde."
                    )
                    st.rerun()

            else:
                st.error(
                    "No se encontró la dirección en la Región Metropolitana. "
                    "Para mayor precisión, pega las coordenadas desde Google Maps."
                )

        except Exception as e:
            st.warning(
                "El servicio de búsqueda de direcciones no respondió correctamente. "
                "Usa coordenadas de Google Maps para mayor precisión."
            )
